...
- R1. Create `uplang/docs.isnow.go` and `uplang/docs.isnow.js` from nicerobot/template.repo-docs (private, Hugo at root, self-gating pages.yml untouched), with `baseURL`/`title`/`description` set.
...
- R3. `uplang/_admin/manifest.yaml`: add `isnow.js` → `docs: docs.isnow.js` override with a docs-reason (non-Go implementation, matching the up.js entry's model).
