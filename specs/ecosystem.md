...
| --- | --- | --- |
| language-core (terminology, conformance corpus, generation retargeting) | `uplang/isnow` (this repo) | [capabilities/language-core.md](capabilities/language-core.md) |
| go-engine (parse → AST → canonicalize → match → derive) | `uplang/isnow.go` root package | [capabilities/go-engine.md](capabilities/go-engine.md) |
| go-cli (the `isnow` binary) | `uplang/isnow.go` `cmd/isnow` + `internal/` | [capabilities/go-cli.md](capabilities/go-cli.md) |
| go-server (`isnow serve` HTTP API) | `uplang/isnow.go` | [capabilities/go-server.md](capabilities/go-server.md) |
| js-engine (`@uplang/isnow`) | `uplang/isnow.js` | [capabilities/js-engine.md](capabilities/js-engine.md) |
| web-playground | `uplang/www.uplang.org` `/isnow/` | [capabilities/web-playground.md](capabilities/web-playground.md) |
| docs-governance (docs repos, manifest entries) | `uplang/docs.isnow.go`, `uplang/docs.isnow.js`, `uplang/_admin` | [capabilities/docs-governance.md](capabilities/docs-governance.md) |
...
- R3. `uplang/isnow.go` implements the full semantics (§6 of the spec) and ships the `isnow` CLI per template.cli standards: urfave/cli/v3, go-error sentinels, shared tools.repository Makefile gate green (fmt, lint, staticcheck, gocognit ≤ 7, govulncheck, 100% coverage of owned packages).
...
- R6. `uplang/isnow.js` implements the same semantics with the same conformance corpus, packaged as `@uplang/isnow` per up.js conventions (node --test, 100% line coverage of the residue, eslint complexity ≤ 7).
...
- AC6. All new repos exist under uplang (private), push to `main`, and their CI workflows are green.
